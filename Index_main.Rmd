...
df <- MASTER <- read_excel(here::here("DATA/MASTER.xlsx")) %>% as_tibble() %>% 
  dplyr:: filter(locality != "Ohu2") %>% 
...
# Remove NAs
df_long <- df %>%
  pivot_longer(cols = c(CAT_sp, PAR_sp), names_to = "Group", values_to = "Species") %>%
  filter(!is.na(Species))

# Count species abundances separately for caterpillars and parasitoids
abundance_data <- df_long %>%
  group_by(Group, Species) %>%
  summarise(count = n(), .groups = "drop") %>%
  pivot_wider(names_from = Species, values_from = count, values_fill = 0)

# Convert to iNEXT input format
caterpillar_abundance <- as.numeric(abundance_data[abundance_data$Group == "CAT_sp", -1])
parasitoid_abundance <- as.numeric(abundance_data[abundance_data$Group == "PAR_sp", -1])

# Create a list for iNEXT
data_list <- list(Caterpillars = caterpillar_abundance, Parasitoids = parasitoid_abundance)

# Run iNEXT
inext_res <- iNEXT(data_list, q = c(0, 1, 2), datatype = "abundance")

inext_res  # Results of diversity
...
t.test(BC_par$hodnota , BC_cat$hodnota, var.equal = TRUE)
...
t.test(CS_par$hodnota , CS_cat$hodnota, var.equal = TRUE)
```
# Sorensen
...
t.test(SOR_par$hodnota , SOR_cat$hodnota, var.equal = TRUE)
```
...
BC_melt <- dplyr::rename (BC_melt, Locality_A = Var1, Locality_B = Var2)
...
BC_FIG_1 <- ggplot(ind_BC, aes(x = guild, y = hodnota, fill = guild)) +
  ...
  labs(title = "Bray-Curtis") +  # Keep only one legend for 'fill'
...
CS_FIG_1 <- ggplot(ind_CS, aes(x = guild, y = hodnota, fill = guild)) +
  ...
  scale_color_manual(values = c("Parasitoids" = "black", 
                               ...
                               "Caterpillars-subsampled" = "black"))  +
  coord_cartesian(ylim = c(0, 1)) +
  labs(title = "Chao-Sorensen", ) +  # Keep only one legend for 'fill'
...
SOR_FIG_1 <- ggplot(ind_SOR, aes(x = guild, y = hodnota, fill = guild)) +
  ...
  scale_color_manual(values = c("Parasitoids" = "black", 
                               ...
                               "Caterpillars-subsampled" = "black"))  +
  coord_cartesian(ylim = c(0, 1)) +
  labs(title = "Sorensen") +  # Keep only one legend for 'fill'
...
Fig_S1_final <-ggarrange(BC_FIG_1, CS_FIG_1, SOR_FIG_1, common.legend = TRUE, legend = "bottom", nrow = 1)
Fig_S1_final
ggsave("output/fig/Fig_S1_final.tiff",
...
BC_FIG_1_simple <- ggplot(ind_BC_simple, aes(x = guild, y = hodnota, fill = guild)) +
  ...
  scale_color_manual(values = c("Parasitoids" = "black", 
                               "Caterpillars" = "black"))  +
  coord_cartesian(ylim = c(0, 1)) +
  labs(title = "Bray-Curtis", x = "Guild", y = "Dissimilarity") +  # Keep only one legend for 'fill'
...
CS_FIG_1_simple <- ggplot(ind_CS_simple, aes(x = guild, y = hodnota, fill = guild)) +
  ...
  scale_color_manual(values = c("Parasitoids" = "black", 
                               "Caterpillars" = "black"))  +
  coord_cartesian(ylim = c(0, 1)) +
  labs(title = "Chao-Sorensen", x = "", y = "", fill = "") +  # Keep only one legend for 'fill'
...
SOR_FIG_1_simple <- ggplot(ind_SOR_simple, aes(x = guild, y = hodnota, fill = guild)) +
  ...
  scale_color_manual(values = c("Parasitoids" = "black", 
                               "Caterpillars" = "black"))  +
  coord_cartesian(ylim = c(0, 1)) +
  labs(title = "Sorensen", x = "", y = "", fill = "") +  # Keep only one legend for 'fill'
