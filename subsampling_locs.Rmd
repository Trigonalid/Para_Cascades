...
subsample_once <- function(data, data_locality_summary, replace = TRUE) {
  subsampled_data <-
    data %>%
    dplyr::group_by(locality) %>%
    dplyr::group_modify(~ {
      ...
      dplyr::sample_n(
        .x,
        size = unique(.x$abundance_caterpillars),
        replace = replace
      )
    }) %>%
    dplyr::ungroup()
...
subsampled_results <-
  purrr::map(
    .progress = TRUE,
    .x = 1:99,
    .f = ~ subsample_once(merged_data, data_locality_summary)
  ) %>%
  dplyr::bind_rows(subsampled_results, .id = "iteration")
...
subsampled_results <-
  purrr::map(
    .progress = TRUE,
    .x = 1:99,
    .f = ~ subsample_once(merged_data, data_locality_summary, replace = FALSE)
  ) %>%
  dplyr::bind_rows(subsampled_results, .id = "iteration")
# Step 4: Calculate the mean interactions across all 99 iterations
mean_subsampled_interactions <-
