...
EFS_A <-
  ...
  scale_x_discrete(labels = c("Caterpillar sp.")) + # Colorblind-friendly colors
  labs(
    x = "",
    y = "Mean Number of localities without parasitoids",
    title = ""
  +
  coord_cartesian(ylim = c(0, 1)) ) +
  theme_minimal() +
  theme(legend.position = "none",axis.text.x = element_text(color = "black")) ;EFS_A
...
EFS_B <-
  ...
  scale_x_discrete(labels = c("Caterpillar + Parasitoid sp.")) +
  labs(
    x = "",
    y = "Mean Number of localities without parasitoids",
    title = ""
...
combined_plot <- ggarrange(ggarrange(EFS_A, EFS_B, ncol = 2, labels = c("A", "B"))) 
EFS_combined_plot <- annotate_figure(combined_plot, top = text_grob("Enemy free space for caterpillars", 
              , face = "bold", size = 16))
